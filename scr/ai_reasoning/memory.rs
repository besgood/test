...
pub fn analysis_prompt(scan_summary: &str) -> String {
    format!(
        r#"You are a penetration testing assistant. Analyze the following scan results and suggest next steps.
...
pub fn exploit_prompt(vuln_detail: &str) -> String {
    format!(
        r#"You are a security expert. Given this vulnerability detail, what specific commands, payloads, or tools would you run?
...
pub fn risk_summary_prompt(context: &str) -> String {
    format!(
        r#"Based on the following information, rate the severity of the risk (1–10) and briefly justify:
